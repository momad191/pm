...
import {
  RiskStatus,
} from 'src/risk/schemas/risk.schema';
...
export class DashboardService {
  ...
  async getWorkload(companyId: string) {
    ...
    const sprintVelocity = await this.sprintModel.aggregate([
      ...
      // -----------------------------------------------------
      // Get Only Tasks Belonging to Same Company
      // -----------------------------------------------------
  ...
  return {
    ...
    projects: {
      ...
      averageCompletion: Number(
        (project.averageCompletion || 0).toFixed(2),
      ),
    },
...
async getManagerDashboard(
 ...
 async getTeamDashboard(
  ...
  const todo =
    taskSummary.find((t) => t._id === 'TODO')?.count || 0;
...
async getRiskAnalysis(companyId: string) {
  ...
  const [
    ...
    this.riskModel.aggregate([
      ...
      {
        $match: {
          'project.companyId': companyObjectId,
          'project.isDeleted': false,
        },
      },
    ...
    this.riskModel.aggregate([
      ...
      {
        $match: {
          'task.companyId': companyObjectId,
   ...
   * ----------------------------------------
   */
  const totalRisks = await this.riskModel.countDocuments({
    companyId: companyObjectId,
...
async getPerformance(companyId: string) {
  const companyObjectId = new Types.ObjectId(companyId);
